/* =
	...
	changes made for BPT Wordpress plugin
	switched getYear, getMonth, getDate, etc to
	their getUTC equivalents.
*/
			...
			if (data.length) {
				...
				jQuery.each(data, function(key, event) {
					if (eventsOpts.jsonDateFormat == 'human') {
						...
						var ampm = eventHour >= 12 ? 'pm' : 'am';
							eventHour = eventHour % 12;
							eventHour = eventHour ? eventHour : 12;
							eventMinute = eventMinute < 10 ? '0'+eventMinute : eventMinute;
						var twelveHourFormat = eventHour + ':' + eventMinute + ' ' + ampm;
					...
					if (limit === 0 || limit > i) {
						...
						if ((month === false || month == eventMonth)
								...
								if (month === false && eventDate < new Date()) {
									...
									events.push('<li id="' + key + '" class="'+event.type+'"><time datetime="'+eventDate+'"><em>' + eventStringDate + '</em><small>'+twelveHourFormat+'</small></time>'+eventTitle+'<p class="eventDesc ' + eventDescClass + '">' + event.description + '</p></li>');
									i++;
								}
						}
					}
